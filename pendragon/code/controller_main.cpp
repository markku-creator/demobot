/**
  ...
  The Pendragon conroller example here uses static IO device configuration. This means that
  communication signal map from IO board JSON files, etc, is compiled into Pendragon's code ->
  run time matching IO signal at IO device and in Pendragon is by address and type, not by signal name.
...
osalStatus osal_main(
    ...
    os_char *argv[])
{
    const os_char *device_name = "pendragon";
    ...
    /* Create pendragon main object and start listening for clients.
     */
